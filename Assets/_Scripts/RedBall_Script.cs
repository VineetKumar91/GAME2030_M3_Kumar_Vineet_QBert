...
public class RedBall_Script : BallMovement
{
    [SerializeField]
    ...
    private bool killRedBallExecuteOnce = true;
    ...
    void FixedUpdate()
    {
        ...
        if (this.transform.position.y < -4)
        {
            ...
            if (this.transform.position.y <= -6 && killRedBallExecuteOnce)
            {
                StartCoroutine(DestroyRedBall());
                killRedBallExecuteOnce = false;
            }
        }
    }
